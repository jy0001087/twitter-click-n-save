...
// 定义文件名最大长度（字符数）
const filenamelength = 70;
// 定义文件名最大字节数（用于 Android/文件系统 255 字节限制）
 ...
 * */
const imageFilenameTemplate      = `[x]{author}—{tweetText} —{name}.{extension}`;
const videoFilenameTemplate      = `[x]{author}—{tweetText} —{name}.{extension}`;
const backgroundFilenameTemplate = `[x][bg] {username}—{id}—{seconds}.{extension}`;
...
<div class="ujs-btn-common ujs-btn-error-text"></div>`.trimStart();
    ...
    class Core {
        /**
         * Sanitizes a string to be safe for use as a filename across various operating systems.
         * Replaces problematic characters with an underscore.
         * @param {string} text - The string to sanitize.
         * @returns {string} The sanitized string.
         */
        static _sanitizeFilename(text) {
            // Characters generally problematic for filenames across Windows, macOS, and Linux/Android.
            // Windows: \ / : * ? " < > |
            // macOS: : (historically, though modern macOS is more flexible, still good practice for cross-platform)
            ...
            // 当文件名字节数超过限制时，逐步缩短
            if (byteLen(sanitizedText) > maxFilenameBytes) {
                // 1. 删除所有的#以及#所带的tag内容。
                // Removes hashtags (e.g., #tag, #another_tag_123)
                sanitizedText = sanitizedText.replace(/#[\p{L}\p{N}_]+/gu, '').trim();
                sanitizedText = sanitizedText.replace(/\s+/g, ',').replace(/^,+|,+$/g, '');

                if (byteLen(sanitizedText) > maxFilenameBytes) {
                    // 2. 每个emoji在文件名中只出现一次（保留首次出现，删除后续重复）。
                    // Each emoji appears only once in the filename (keep first, remove later duplicates).
                    {
                        const seen = new Set();
                        sanitizedText = sanitizedText.replace(/\p{Emoji_Presentation}|\p{Extended_Pictographic}/gu, m => seen.has(m) ? '' : (seen.add(m), m));
                    }
                    sanitizedText = sanitizedText.replace(/\s+/g, ',').replace(/^,+|,+$/g, '');
                    ...
                    if (byteLen(sanitizedText) > maxFilenameBytes) {
                        ...
                        if (byteLen(sanitizedText) > maxFilenameBytes) {
                            // 4. 删除营销文案（求关注！求点赞！等）
                            sanitizedText = sanitizedText.replace(/ 求关注！求点赞！完全免费录制！/g, '');
                            sanitizedText = sanitizedText.replace(/ 求关注！求点赞！/g, '');
                            sanitizedText = sanitizedText.replace(/\s+/g, ',').replace(/^,+|,+$/g, '');

                            if (byteLen(sanitizedText) > maxFilenameBytes) {
                                // 5. 删除元信息：删除"作者："到"tips："之间的内容（保留tips）
                                sanitizedText = sanitizedText.replace(/ 作者：.+?tips：/g, ' tips：');
                                sanitizedText = sanitizedText.replace(/\s+/g, ',').replace(/^,+|,+$/g, '');

                                if (byteLen(sanitizedText) > maxFilenameBytes) {
                                    // 6. 删除中文标点符号
                                    sanitizedText = sanitizedText.replace(/[，。、：；？！“”‘’【】——……·「」『』—…@＃#]/gu, '').trim();
                                    sanitizedText = sanitizedText.replace(/\s+/g, ',').replace(/^,+|,+$/g, '');

                                    if (byteLen(sanitizedText) > maxFilenameBytes) {
                                        // 7. 删除所有Unicode标点符号（原有规则）
                                        sanitizedText = sanitizedText.replace(/[\p{P}]/gu, '').trim();
                                        sanitizedText = sanitizedText.replace(/\s+/g, ',').replace(/^,+|,+$/g, '');

                                        if (byteLen(sanitizedText) > maxFilenameBytes) {
                                            // 8. 按字节长度安全截断（不会截断中文字符中间）
                                            let truncated = '';
                                            let count = 0;
                                            for (const ch of sanitizedText) {
                                                const charBytes = new TextEncoder().encode(ch).length;
                                                if (count + charBytes > maxFilenameBytes) break;
                                                truncated += ch;
                                                count += charBytes;
                                            }
         ...
         * @returns {Promise<string>} The tweet's text content, or an empty string if not found.
         */
        static async _getTweetText(tweetId) {
            try {
                const medias = await API.getTweetMedias(tweetId);
                if (medias && medias.length > 0 && medias[0].tweet_text) {
                    console.log("[ujs][_getTweetText] Found tweet text:", medias[0].tweet_text); 
                    const originalText = medias[0].tweet_text;
                    const sanitizedText = Core._sanitizeFilename(originalText);
                    return sanitizedText;
                } else {
                    console.log("[ujs][_getTweetText] No tweet text found for ID:", tweetId); 
                }
            } catch (err) {
                console.error("[ujs][_getTweetText] Failed to get tweet text for ID:", tweetId, err);
            }
            return "";
        }
        ...
        static async _downloadPhotoMediaEntry(id, author, url, btn) {
            ...
            const filename = renderTemplateString(imageFilenameTemplate, {
                author, lastModifiedDate, tweetId: id, name, extension, sampleText, tweetText // 添加 tweetText
            }).value;
...
function getUtils({verbose}) {
    ...
    function downloadBlob(blob, name, url) {
        ...
        // ---- Desktop (unchanged): A2 blob → A3 blobUrl → A1 original URL ----
        ...
        // A1: Original URL + custom name + page Referer (re-download via the browser's downloader)
